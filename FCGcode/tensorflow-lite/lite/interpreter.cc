...
Interpreter::Interpreter(ErrorReporter* error_reporter)
  ...
  for (int i = 0; i < kTfLiteMaxExternalContexts; ++i) {
    ...
    external_contexts_cpu1_[i] = nullptr;
  }
  ...
  own_external_cpu_backend_context_cpu1_.reset(new ExternalCpuBackendContext());
  external_contexts_[kTfLiteCpuBackendContext] =
      own_external_cpu_backend_context_.get();
  external_contexts_cpu1_[kTfLiteCpuBackendContext] =
      own_external_cpu_backend_context_cpu1_.get();
}
...
void Interpreter::SetExternalContext(TfLiteExternalContextType type,
  ...
  primary_subgraph().SetExternalContext1(type, ctx,ctx2);
}
  ...
  for (int i = 0; i < subgraphs_to_add; ++i) {
    Subgraph* subgraph =
        new Subgraph(error_reporter_, external_contexts_, &subgraphs_,
                     &resources_, &resource_ids_, &initialization_status_map_,external_contexts_cpu1_);
...
TfLiteStatus Interpreter::Invoke_2() {
  ScopedRuntimeInstrumentationProfile scoped_runtime_event(installed_profiler_,
                                                           "invoke2");
  ...
  TF_LITE_ENSURE_STATUS_WITH_SCOPED_INSTRUMENTATION(
      scoped_runtime_event, primary_subgraph().Invoke_2());
