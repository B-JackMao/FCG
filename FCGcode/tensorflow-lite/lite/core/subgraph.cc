...
Subgraph::Subgraph(ErrorReporter* error_reporter,
                   ...
                   resource::InitializationStatusMap* initialization_status_map,TfLiteExternalContext** external_contexts1)
    : external_contexts_(external_contexts),
      external_contexts_cpu1_(external_contexts1),
  ...
  cpu1_context_.impl_ = static_cast<void*>(this);
  cpu1_context_.ResizeTensor = ResizeTensor;
  cpu1_context_.ReportError = ReportErrorC;
  cpu1_context_.AddTensors = AddTensors;
  cpu1_context_.tensors = nullptr;
  cpu1_context_.tensors_size = 0;
  cpu1_context_.allow_fp32_relax_to_fp16 = false;
  cpu1_context_.recommended_num_threads = -1;
  cpu1_context_.GetExternalContext = GetExternalContext1;
  cpu1_context_.SetExternalContext = SetExternalContext;
  cpu1_context_.profiler = nullptr;
  cpu1_context_.GetTensor = nullptr;
  cpu1_context_.GetEvalTensor = nullptr;
  cpu1_context_.GetModelMetadata = GetModelMetadata;
  cpu1_context_.GetNodeName = GetNodeName;
...
//std::cout<<"88888888888888880"<<std::endl;
  for (auto& node_subset : node_subsets) {
    ...
    //std::cout<<"node_subset.type:"<<node_subset.type<<std::endl;
...
TfLiteExternalContext* Subgraph::GetExternalContext1(
    TfLiteExternalContextType type) {
  if (static_cast<int>(type) >= 0 && type < kTfLiteMaxExternalContexts) {
    return external_contexts_cpu1_[type];
  }
  return nullptr;
}
TfLiteExternalContext* Subgraph::GetExternalContext1(
    struct TfLiteContext* context, TfLiteExternalContextType type) {
      //std::cout<<"++++++++++"<<std::endl;
  return static_cast<Subgraph*>(context->impl_)->GetExternalContext1(type);
}
...
void Subgraph::SetExternalContext1(TfLiteExternalContextType type,
                                  TfLiteExternalContext* ctx,TfLiteExternalContext* ctx2) {
  if (static_cast<int>(type) >= 0 && type < kTfLiteMaxExternalContexts) {
    external_contexts_[type] = ctx;
    external_contexts_cpu1_[type] = ctx2;
  }
}

void Subgraph::SetExternalContext1(struct TfLiteContext* context,
                                  TfLiteExternalContextType type,
                                  TfLiteExternalContext* ctx,TfLiteExternalContext* ctx2) {
  return static_cast<Subgraph*>(context->impl_)->SetExternalContext1(type, ctx,ctx2);
}
...
TfLiteStatus Subgraph::PrepareOpsAndTensors() {
  std::cout<<"PrepareOpsAndTensors"<<std::endl;
...
TfLiteStatus Subgraph::Invoke() {
  ...
  std::vector<std::chrono::microseconds> gpu_wait_ms, dsp_wait_ms,cpu1_wait_ms;
std::vector<uint64_t> stage_time_ms;

  std::future<void> dsp_future;
  std::future<void> cpu1_future;
  std::future<void> cpu2_future;
  unsigned char state = kMPFlagSeq;
  std::vector<int> cpu_executing;
  std::vector<int> gpu_executing;
  const TfLiteRegistration* gpu_registration = nullptr;
   TfLiteNode* next_gpu_node =nullptr;
  TfLiteNode* gpu_node;
  int gpu_index = 0;
  ...
  // called.
      ...
      if (mp_flags_.find(node_index) == mp_flags_.end()) {
        ...
        || (node_name.find("StatefulPartitionedCall") != std::string::npos)
          ...
          flag |= kMPFlagDsp;
          enable_dsp_ = true;
        } else if (node_name.find("cpu1") != std::string::npos) {
          flag |= kMPFlagCpu1;
          enable_cpu1_ = true;
        }else {
          flag |= kMPFlagCpu;
  ...
  int8_t* test=(int8_t*)context_.tensors[16].data.data;
  int8_t* test1=new int8_t[sizeof(*test)];
  uint64_t end_time;
  uint64_t start_time;
  uint64_t start_time1;
  uint64_t end_stage;
  uint64_t start_stage;
  start_time1 = NowMillis();
  for (int execution_plan_index = 0;
       execution_plan_index < execution_plan_.size(); execution_plan_index++) {
    int node_index = execution_plan_[execution_plan_index];
    TfLiteNode& node = nodes_and_registration_[node_index].first;
    ...
    if(node_name.find("mp_end") != std::string::npos){
      ...
      for (int cpu_node_index: cpu_executing) {
        ...
        cpu_registration.invoke(&context_,&cpu_node);
      }
       cpu_executing.clear();

        
        if(gpu_registration!=nullptr){

          auto start_gpu = std::chrono::high_resolution_clock::now();
          gpu_registration->wait_for_completion(&context_,next_gpu_node);
          auto end_gpu = std::chrono::high_resolution_clock::now();
          gpu_wait_ms.push_back(std::chrono::duration_cast<std::chrono::microseconds>(end_gpu - start_gpu));

          end_stage=NowMillis();
          stage_time_ms.push_back(end_stage-start_stage);
          gpu_registration = nullptr;

        }

      auto start_connect_from_gpu = std::chrono::high_resolution_clock::now();
      if (OpInvoke(registration, &node) != kTfLiteOk) {
          return ReportOpError(&context_, node, registration, node_index,
                              "failed to invoke");
      }
      auto end_connect_from_gpu = std::chrono::high_resolution_clock::now();
      auto duration_connect_from_gpu = std::chrono::duration_cast<std::chrono::microseconds>(end_connect_from_gpu - start_connect_from_gpu);

        if (node_name.find("last_mp_mp_start") != std::string::npos){

         start_time = NowMillis();
        }
    }
    else if (node_name.find("mp_start") != std::string::npos) {
        for (int cpu_node_index: cpu_executing) {
        TfLiteNode& cpu_node = nodes_and_registration_[cpu_node_index].first;
        const TfLiteRegistration& cpu_registration = 
          nodes_and_registration_[cpu_node_index].second;  
