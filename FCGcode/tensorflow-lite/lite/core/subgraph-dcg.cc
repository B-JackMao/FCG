...
TfLiteStatus Subgraph::Invoke() {
  ...
  for (int execution_plan_index = 0;
      ...
      for (int cpu_node_index: cpu_executing) {
       ...
       cpu_executing.clear();
        if(gpu_registration!=nullptr){
          ...
          //std::cout<<"gpu:"<<std::chrono::duration_cast<std::chrono::microseconds>(end_gpu - start_gpu).count()<<std::endl;
          gpu_registration = nullptr;
        }

      if (OpInvoke(registration, &node) != kTfLiteOk) {//运行CPU结点
    ...
    else if (node_name.find("mp_start") != std::string::npos) {//再判断是否是开头结尾结点
        for (int cpu_node_index: cpu_executing) {
        TfLiteNode& cpu_node = nodes_and_registration_[cpu_node_index].first;
        const TfLiteRegistration& cpu_registration = 
          nodes_and_registration_[cpu_node_index].second;  
  ...
  //std::cout<<"start:"<<start_time<<" ,endtime:"<<end_time<<" ,all2_time:"<<end_time-start_time<<std::endl;
  } // TFLITE_SCOPED_TAGGED_DEFAULT_PROFILE(profiler_.get(), "Invoke");
...
TfLiteStatus Subgraph::Invoke_2() {
 context_.num=3;
     cpu_set_t mask;
    CPU_ZERO(&mask);    /* 初始化set集，将set置为空*/
    CPU_SET(3, &mask);
 ...
 //std::cout<<"start_2:"<<start_2<<std::endl;
  for (int execution_plan_index = start_2+1;
       execution_plan_index < execution_plan_.size(); execution_plan_index++) {
    int node_index = execution_plan_[execution_plan_index];
    TfLiteNode& node = nodes_and_registration_[node_index].first;
    const TfLiteRegistration& registration =
        nodes_and_registration_[node_index].second;
    tensor_resized_since_op_invoke_ = false;
    //std::cout<<"invoke_2"<<std::endl;
    //std::cout<<"invoke2:"<<context_.num<<std::endl;
    if (OpInvoke(registration, &node) != kTfLiteOk) {//运行CPU结点
        return ReportOpError(&context_, node, registration, node_index,
                            "failed to invoke");
    }
