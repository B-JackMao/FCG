...
// void load_file(const std::string& file_path, std::vector<uint8_t>& data) {
...
//     std::memcpy(input_tensor->data.uint8, image_data.data(), image_data.size());
...
// const uint8_t* output_data_uint8 = output_tensor->data.uint8;
...
//     //std::cout << "Predicted class: " << std::distance(output_tensor->data.data, std::max_element(output_tensor->data.data, output_tensor->data.data + output_tensor->bytes / sizeof(uint8_t))) << std::endl;
...
// bool decodeJPEG(const std::string& filename, std::vector<uint8_t>& imageData, int& width, int& height, int& channels) {
...
// std::vector<uint8_t> addBatchDimension(const std::vector<uint8_t>& imageData, int height, int width, int channels) {
//     std::vector<uint8_t> batchedImage(1 * height * width * channels);
//     std::memcpy(batchedImage.data(), imageData.data(), imageData.size());
//     return batchedImage;
// }

// // Updated resizeImage function for NHWC format
// std::vector<uint8_t> resizeImage(const std::vector<uint8_t>& inputImage, int inputWidth, int inputHeight, int channels, int targetWidth, int targetHeight) {
//     std::vector<uint8_t> outputImage(targetHeight * targetWidth * channels);
...
// bool runInference(const std::vector<uint8_t>& imageData, int width, int height, int channels, const std::string& modelPath) {
//     int targetHeight = 224;
//     int targetWidth=224;
//     std::vector<uint8_t> resizedImage = resizeImage(imageData, width, height, channels, targetWidth, targetHeight);
//     std::vector<uint8_t> batchedImage = addBatchDimension(resizedImage, targetHeight, targetWidth, channels);
//         // 替换为你的预处理后图像文件路径和TFLite模型文件路径
//     // 2. Initialize TensorFlow Lite interpreter
//     std::unique_ptr<tflite::FlatBufferModel> model =
...
//     int input_tensor_idx = interpreter->inputs()[0];
//     TfLiteTensor* input_tensor = interpreter->tensor(input_tensor_idx);
//     std::memcpy(input_tensor->data.uint8, batchedImage.data(), batchedImage.size());
...
//     // 读取数据
//     std::vector<uint8_t> input_data1 = readDataFromFile("input1_data.txt");
//     std::vector<uint8_t> input_data2 = readDataFromFile("input2_data.txt");
//     std::vector<uint8_t> input_data3 = readDataFromFile("input3_data.txt");
...
//     std::memcpy(tensor1->data.uint8, input_data1.data(), input_data1.size());
//     std::memcpy(tensor2->data.uint8, input_data2.data(), input_data2.size());
//     std::memcpy(tensor3->data.uint8, input_data3.data(), input_data3.size());
// // 运行推理
...
//    //uint8_t* output_data_uint8 = interpreter->typed_output_tensor<uint8_t>(output_tensor_idx);
//     int output_size = output_tensor->dims->data[1]; // 假设输出是一个1D数组
...
// std::vector<uint8_t> convertToUint8Vector(short* band_dct, int dct_h, int dct_w, int dct_b) {
...
// // Function to write vector<uint8_t> data to a text file
...
// }
// // 双线性插值函数
// uint8_t bilinearInterpolate(const std::vector<uint8_t>& input, int inputWidth, int inputHeight, float x, float y, int channel, int inputChannels) {
...
// ) {
//     std::vector<uint8_t> outputData(outputWidth * outputHeight * inputChannels);
...
//     // 转换 uint8_t* 数据到 std::vector<uint8_t>
//     //std::vector<uint8_t> input1 = convertToUint8Vector(band1_dct, band1_dct_h, band1_dct_w, band1_dct_b);
//     //std::vector<uint8_t> input2 = convertToUint8Vector(band2_dct, band2_dct_h, band2_dct_w, band2_dct_b);
//     //std::vector<uint8_t> input3 = convertToUint8Vector(band3_dct, band3_dct_h, band3_dct_w, band3_dct_b);
//     std::vector<uint8_t> input_data1(band1_dct_h * band1_dct_w * band1_dct_b);
//     std::vector<uint8_t> input_data2(band2_dct_h * band2_dct_w * band2_dct_b);
//     std::vector<uint8_t> input_data3(band3_dct_h * band3_dct_w * band3_dct_b);
...
//     std::vector<uint8_t> resizedImage1 = resizeImage(input_data1, band1_dct_w, band1_dct_h, band1_dct_b, targetWidth1, targetHeight1);
//     std::vector<uint8_t> resizedImage2 = resizeImage(input_data2, band2_dct_w, band2_dct_h, band2_dct_b, targetWidth2, targetHeight2);
//     std::vector<uint8_t> resizedImage3 = resizeImage(input_data3, band3_dct_w, band3_dct_h, band3_dct_b, targetWidth2, targetHeight2);


//     std::vector<uint8_t> input1_batch = addBatchDimension(resizedImage1,targetHeight1, targetWidth1, band1_dct_b);
//     std::vector<uint8_t> input2_batch = addBatchDimension(resizedImage2,targetHeight2, targetWidth2, band2_dct_b);
//     std::vector<uint8_t> input3_batch = addBatchDimension(resizedImage3,targetHeight2, targetWidth2, band3_dct_b);
...
//     std::memcpy(tensor1->data.uint8, input1_batch.data(), input1_batch.size());
//     std::memcpy(tensor2->data.uint8, input2_batch.data(), input2_batch.size());
//     std::memcpy(tensor3->data.uint8, input3_batch.data(), input3_batch.size());
...
//                 outputImage[(y * targetWidth + x) * channels + c] = static_cast<uint8_t>(value);
//             }
//         }
//     }
//     return outputImage;
...
// void runInference(const std::vector<uint8_t>& input1_batch, 
//                               const std::vector<uint8_t>& input2_batch, 
//                               const std::vector<uint8_t>& input3_batch) {
//     int input_index1 = interpreter->inputs()[0];
//     int input_index2 = interpreter->inputs()[1];
//     int input_index3 = interpreter->inputs()[2];
...
//         int targetWidth2 = 14;
//         int targetHeight2 = 14;
//         std::vector<uint8_t> resizedImage1 = resizeImage(input1, band1_dct_w, band1_dct_h, band1_dct_b, targetWidth1, targetHeight1);
//         std::vector<uint8_t> resizedImage2 = resizeImage(input2, band2_dct_w, band2_dct_h, band2_dct_b, targetWidth2, targetHeight2);
//         std::vector<uint8_t> resizedImage3 = resizeImage(input3, band3_dct_w, band3_dct_h, band3_dct_b, targetWidth2, targetHeight2);
...
// std::vector<uint8_t> mergeInputs(const std::vector<uint8_t>& input1, const std::vector<uint8_t>& input2) {
//     // 检查输入大小是否匹配
...
//     std::vector<uint8_t> merged_input(height * width * (depth1 + depth2));
...
//                               const std::vector<uint8_t>& input2_batch, 
//                               const std::vector<uint8_t>& input3_batch) {
//     std::vector<uint8_t> merged_input = mergeInputs(input2_batch, input3_batch);
//     int input_index1 = interpreter->inputs()[0];
//     int input_index2 = interpreter->inputs()[1];
//     //int input_index3 = interpreter->inputs()[2];
...
//     std::memcpy(tensor1->data.uint8, input1_batch.data(), input1_batch.size());
//     std::memcpy(tensor2->data.uint8, merged_input.data(), merged_input.size());
//     //std::memcpy(tensor3->data.uint8, input3_batch.data(), input3_batch.size());
//     // 4. Run inference
//     interpreter->Invoke();
...
//         std::vector<uint8_t> input1 = convertToUint8Vector(band1_dct, targetWidth1, targetHeight1, band1_dct_b);
//         std::vector<uint8_t> input2 = convertToUint8Vector(band2_dct, targetWidth2, targetHeight2, band2_dct_b);
//         std::vector<uint8_t> input3 = convertToUint8Vector(band3_dct, targetWidth2, targetHeight2, band3_dct_b);

//         std::vector<uint8_t> input1_batch = addBatchDimension(input1,targetHeight1, targetWidth1, band1_dct_b);
//         std::vector<uint8_t> input2_batch = addBatchDimension(input2,targetHeight2, targetWidth2, band2_dct_b);
//         std::vector<uint8_t> input3_batch = addBatchDimension(input3,targetHeight2, targetWidth2, band3_dct_b);
...
//     return outputImage;
// }
// void inferenceTask(const std::vector<uint8_t>& imageData, int width, int height, int channels) {
//     int targetHeight = 224;
//     int targetWidth = 224;
//     std::vector<uint8_t> resizedImage = resizeImage(imageData, width, height, channels, targetWidth, targetHeight);
//     std::vector<uint8_t> batchedImage = addBatchDimension(resizedImage, targetHeight, targetWidth, channels);
...
// // };
//     char* files[8] = {
//         "/data/local/tmp/83.jpeg", "/data/local/tmp/84.jpeg", "/data/local/tmp/85.jpeg",
//         "/data/local/tmp/86.jpeg", "/data/local/tmp/87.jpeg", "/data/local/tmp/88.jpeg",
//         "/data/local/tmp/89.jpeg", "/data/local/tmp/90.jpeg"
//     };
//  std::vector<uint8_t> imageData;
//     int width, height, channels;
...
// std::queue<std::tuple<std::string, std::vector<uint8_t>, int, int, int,int>> task_part_queue[3]; // Task queue for each part
...
// void inferenceTask(const std::vector<uint8_t>& imageData, int width, int height, int channels,int id) {
//     int targetHeight = 224;
//     int targetWidth = 224;
...
//     while (true) {
//         std::tuple<std::string, std::vector<uint8_t>, int, int, int,int> task;
//         {
...
//         std::vector<uint8_t>& imageData = std::get<1>(task);
...
const int depth1 = 64;  // 第一个输入的通道数
const int depth2 = 64;  // 第二个输入的通道数
std::vector<int8_t> mergeInputs(const std::vector<int8_t>& input1, const std::vector<int8_t>& input2) {
    // 检查输入大小是否匹配
    if (input1.size() != height * width * depth1 || input2.size() != height * width * depth2) {
        std::cerr << "输入大小不匹配!" << std::endl;
        return {};
    ...
    // 创建合并后的 vector，大小为14x14x128
    std::vector<int8_t> merged_input(height * width * (depth1 + depth2));
...
void inferenceTask(short* band1_dct, int band1_dct_h, int band1_dct_w, int band1_dct_b,
    ...
    std::vector<int8_t> merged_input = mergeInputs(input2_batch, input3_batch);
    ...
    std::memcpy(tensor2->data.int8, merged_input.data(), merged_input.size());
...
int main(int argc, char* argv[]){
    ...
    std::string modelPath = "/data/local/tmp/DCG76ACC.tflite";
    // 替换为你的预处理后图像文件路径和TFLite模型文件路径
    // 2. Initialize TensorFlow Lite interpreter
    std::unique_ptr<tflite::FlatBufferModel> model =
    tflite::FlatBufferModel::BuildFromFile(modelPath.c_str());
    ...
    workers.emplace_back(worker_inference1, 2); // CPU 2 for inferenceTask
    workers.emplace_back(worker_inference2, 3); // CPU 3 for inferenceTask2
    remaining_tasks = std::stoi(argv[2]);
...
//         std::vector<uint8_t> input1 = convertToUint8Vector(resized_data1, targetWidth1, targetHeight1, *band1_dct_b);
//         std::vector<uint8_t> input2 = convertToUint8Vector(resized_data2, targetWidth2, targetHeight2, *band2_dct_b);
//         std::vector<uint8_t> input3 = convertToUint8Vector(resized_data3, targetWidth2, targetHeight2, *band3_dct_b);
...
//     JBLOCK **mcu_buffer = (JBLOCK**)malloc(sizeof(JBLOCK*) * srcinfo->blocks_in_MCU);
//     JBLOCK *block_buffer = (JBLOCK*)inputBuffer;
//     //std::cout<<"totalMcuNum:"<<totalMcuNum<<std::endl;
...
//   auto duration_re = std::chrono::duration_cast<std::chrono::milliseconds>(end_resize - start_resize);
//   //std::cout<<"re:"<<duration_re.count()<<std::endl;
//   if (buffer != nullptr) {
//     delete[] buffer;
//   }
// }
