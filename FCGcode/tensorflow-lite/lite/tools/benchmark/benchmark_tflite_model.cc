...
TfLiteStatus BenchmarkTfLiteModel::InitInterpreter() {
  ...
  if (use_caching) {
    ...
    //-------------------------
    external_context_cpu1_.reset(new tflite::ExternalCpuBackendContext());
    std::unique_ptr<tflite::CpuBackendContext> cpu_backend_context_cpu1(
        new tflite::CpuBackendContext());
    cpu_backend_context_cpu1->SetUseCaching(true);
    cpu_backend_context_cpu1->SetMaxNumThreads(num_threads);
    external_context_cpu1_->set_internal_backend_context(
        std::move(cpu_backend_context_cpu1));
    //------------------------------
    interpreter_->SetExternalContext(kTfLiteCpuBackendContext,
                                     external_context_.get(),external_context_cpu1_.get());
  */}
