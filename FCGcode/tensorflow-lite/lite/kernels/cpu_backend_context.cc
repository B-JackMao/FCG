...
CpuBackendContext* CpuBackendContext::GetFromContext(TfLiteContext* context) {
  ...
  if (external_context == nullptr) {
    //std::cout<<"external_context == nullptr"<<std::endl;
    TF_LITE_FATAL(
  ...
  auto* cpu_backend_context = static_cast<CpuBackendContext*>(
      ...
      //std::cout<<"-----------context->num:------------"<<context->num<<std::endl;
      if (cpu_backend_context != nullptr) {
        if (cpu_backend_context->ruy_context_ != nullptr) {
            //std::cout<<"context->num:"<<context->num<<std::endl;
            cpu_backend_context->SetRuyth(context->num);
            //std::cout<<cpu_backend_context->ruy_context_->th<<"cpu_backend_context->ruy_context_->th:"<<&cpu_backend_context->ruy_context_->th<<std::endl;
        }
  ...
  if (cpu_backend_context == nullptr) {
    ...
    cpu_backend_context = new CpuBackendContext(context->num);
...
CpuBackendContext::CpuBackendContext(int i)
...
void CpuBackendContext::SetRuyth(int th) {
  ruy_context_->th=th;
}
