...
void CreateTrMulParamsAssumingColMajorDst(
  ...
  const Path the_path = ctx->SelectPath(CompiledPaths);//选择要使用的计算路径（Path），这个路径信息在ctx上下文中缓存。不同的计算路径可以对应不同的优化实现。CompiledPaths可能包含多个不同的路径
  ...
  PopulateTrMulParamsAllCompiledPaths<CompiledPaths, LhsScalar, RhsScalar,
                                      AccumScalar, DstScalar>(the_path, params);//这个函数用于填充params结构中与所选路径（Path）相关的信息。这包括选择适当的内核函数和其他优化参数。
