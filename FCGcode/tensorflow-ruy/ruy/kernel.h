...
//RunKernel:：Run（）是唯一一个直接调用Kernel::Run（）的地方。

//它执行类型取消擦除，并分解所有Kernel:：Run（）调用

//通过这个函数还提供了一个单独的位置

//实现RUY_OPT（FAT_KERNEL）。这应该是一个函数，但却是

//隐藏并共享一些样板（请参阅成员类型和RunTyped
...
class RunKernel final {
 ...
 private:
  ...
  static void RunTyped(Tuning tuning, const PMat<LhsScalar>& lhs,
    ...
    KernelType kernel(tuning);//这行代码创建一个名为kernel的对象，该对象是计算内核的实例，根据给定的tuning参数进行配置
...
//std::cout<<"kernel.h1"<<std::endl;
  kernel.Run(lhs, rhs, mul_params, start_row, start_col, end_row, end_col, dst);//如果启用了FAT_KERNEL优化，则此行代码直接调用内核的Run函数来执行计算
#else
std::cout<<"kernel.h2"<<std::endl;
